// Disclaimer! I don't know everything. I'm new to coding. It helps me in the moment and when coming back later to write detailed comments. I'm taking this to the next level in this bootcamp so that me and other students can attempt to make more sense of a vague and ambiguous curriculum. 
...
// *** you are creating the name of the parameters as arguments at the same time you're creating the function. You do not need to make ball, x, or y into variables. They're parameters that exist only in this function as ball is not declared anywhere. 
...
// *** once again this is creating the name of the parameters as arguments at the same time you're creating the function. You do not need to make ball, x, or y a variable. They're parameters that exist only in this function. Yes, the same parameter names are being used in moveTo() but they are still functionally scoped as ball is not declared anywhere. 
...
// *** reminder, you could name color, dx, and dy whatever you'd like as long as you refer to them the same way throughout the function's scope. These arguments were not declared anywhere earlier. Although, dx and dy are commonplace to mean directionX and directionY
function create(color, dx, dy) {
  // *** line 58 is object construction. Strangely this comes from way out of left field seeing as how they only just taught us what a primitive data types were this week...
  //  *** anyway... newBall is being created from "this". "this" is pretty advanced JS. What "this" is changes often based on how it's used. In this context, we're telling JS to create a new object with the properties were setting on newBall.___ 
  ...
  // *** dynamically creates a new element called "div" and drops it into the HTML body. This div will house the newBall element
  // *** this is where we create the .element we'll use later. It refers to the new div we created that will be read by HTML. Much like for the "area" Object in line 15
  newBall.element = document.createElement("div");

  // TODO: set the backgroundColor, width and height style properties for newBall.element
  // *** because newBall is an Object, element is one of it's keys. After accessing the element, (which above we set to the "area" div), we're going to use .style.___ to set it's color, width, and height. Concatenating with the string "px" again so CSS knows its a pixel location.
  ...
  // *** because we concatenated our height/width with "px" earlier so CSS could understand it, we now have to take that "px" off so javascript can work with it. The parseInt() JS built-in function removes strings and converts it back to a primitive Number. Because JS is read top to bottom, we are overwriting height and width again here
...
function update(ball, x, y) {
  ...
  // *** call moveTo() function inside the update function. In order to update(), 1st we use the current arguments for ball, x, y to determine where the ball is, 2nd we need to moveTo() the new coordinates
  moveTo(ball, x, y);

  // TODO: use the Javascript setTimeout() method to call changeDirectionIfNecessary() and update() every 16ms
  // *** this feels like a tricky way to word this. We need to find out where the ball is before calling either of those functions on it since these arguments are not pointing to a position on the page.
  // *** let's make x and y mean something inside this function. We'll need them to be defined so we can know where the ball has moved to before calling update() on it.
  x = x + ball.dx;
  y = y + ball.dy;
...
// *** *** *** below is the answer they gave us. Please tell me how a 2 weeks new student would be able to do this with no help, resources, or links? Why do they need to divide 1000 / 60? Why is this answer so convoluted? This is very much giving "POP QUIZ! GOTCHA!" vibes that feels just plain rude to give to a student who's been coding part-time for less than 2 weeks with no explanation of what it means or why. New students don't know if this is supposed to be easy or hard AF. 
// *** *** *** As a beginner, if I were presented with this solution as if it was the simplest thing in the world, I would be scared shitless of the program. I mean c'mon... let us build some confidence first! Is this a scare tactic? I dunno... but the real bummer is tht it's never discussed in class before or after. They throw this at us and don't say a word of clarity or excitement about it later.
//  vvvvvvvvvv
